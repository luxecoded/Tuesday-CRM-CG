...
export default function DealDrawer({ deal, onClose, onSave, isDesktop, contacts = [] }) {
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {form.group === 'won' ? (
            <button
              onClick={() => set('group', 'active')}
              className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-amber-400/20 text-amber-700 dark:bg-amber-400/15 dark:text-amber-300 border border-amber-400/30 dark:border-amber-400/20 hover:bg-amber-400/30 transition-colors">
              Move to Active
            </button>
          ) : (
            <button
              onClick={() => set('group', 'won')}
              className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-green-400/20 text-green-700 dark:bg-green-400/15 dark:text-green-300 border border-green-400/30 dark:border-green-400/20 hover:bg-green-400/30 transition-colors">
              Mark as Won
            </button>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input type="text" value={form.location || ''} onChange={e => set('location', e.target.value)} placeholder="Address or area" className={inputCls} />
          </div>
        </div>

        <div>
          <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">Contact</label>
          <select
            ...
            onChange={e => {
              ...
              set('contactId', id || '')
              set('contact', contact ? contact.name : '')
            }}
            className={inputCls}
          >
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input type="text" value={form.surveyor || ''} onChange={e => set('surveyor', e.target.value)} placeholder="Surveyor name" className={inputCls} />
          </div>
        </div>
